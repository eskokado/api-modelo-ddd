...
namespace Api.Application.Controllers
{
    ...
    public class UfsController : ControllerBase
    {
        ...
        [Authorize("Bearer")]
        [HttpPost]
        public async Task<ActionResult> Post([FromBody] UfDtoCreate dto) 
        {
            if (!ModelState.IsValid) 
            {
                return BadRequest(ModelState);
            ...
            try
            {
                ...
                if (result != null) 
                {
                    return Created(new Uri(Url.Link("GetUfWithId", new {id = result.Id})), result);
                }
                else
                {
                    return BadRequest();
        ...
        [Authorize("Bearer")]
        [HttpPut]
        public async Task<ActionResult> Put([FromBody] UfDtoUpdate dto) 
        {
            if (!ModelState.IsValid) 
            {
                return BadRequest(ModelState);
