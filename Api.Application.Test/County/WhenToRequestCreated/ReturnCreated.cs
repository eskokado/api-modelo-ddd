...
namespace Api.Application.Test.County.WhenToRequestCreated
{
    public class ReturnCreated
    {
        ...
        public async Task It_is_possible_to_create_Created()
        {
            ...
            var resultValue = ((CreatedResult) result).Value as CountyDtoCreateResult; 
            Assert.NotNull(resultValue);
            Assert.Equal(countyDtoCreate.Name, resultValue.Name);
            Assert.Equal(countyDtoCreate.CodeIBGE, resultValue.CodeIBGE);
            Assert.Equal(countyDtoCreate.UfId, resultValue.UfId);
        }
    }
}